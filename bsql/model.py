...
class Model(Record):
    ...
    def to_one(
        ...
        **kwargs
        ...
        if cache_field is None:
            cache_field = "__" + other_class.__name__
        ...
        if update == True or self.__dict__.get(cache_field) == None:
            ...
            if orderby is not None:
                selexpr = (
                    """other_class(self.db).select_one(attr="%s", where="%s", orderby="%s", **kwargs)"""
                    % (attr, wherecl, orderby)
                )
            else:
                selexpr = """other_class(self.db).select_one(attr="%s", where="%s", **kwargs)""" % (
                    attr,
                    wherecl,
                )
        ...
        **kwargs
        ...
        if update == True or self.__dict__.get(cache_field) == None:
            ...
            if where not in ['', None]:
                ...
                wherecl += " and %s " % where
            if orderby is not None:
                selexpr = """other_class(self.db).select(where="%s", orderby="%s", **kwargs)""" % (
                    wherecl,
                    orderby,
                )
            else:
    ...
    def to_many_through(
        ...
        **kwargs
        ...
        if update == True or self.__dict__.get(cache_field) == None:
            ...
            onself = [
                " %s=%s " % field for field in zip(self_key, through_fields)
            ]  # uses the first set of fields in through_fields
            onforeign = [
                " %s=%s " % field for field in zip(other_key, through_fields[len(onself) :])
            ]  # uses the rest of the fields in through_fields

            # build the query
            sql = "select " + ", ".join(
                ["%s.%s" % (other_class.relation, field) for field in to_fields]
            )
            ...
            sql += "\n where %s" % " and ".join(
                ["%s=%s" % (field, self.quote(self[field.split('.')[1]])) for field in self_key]
            )
            if where not in ['', None]:
                sql += " and %s " % where
            if orderby in ['', None]:
                orderby = ",".join(other_class.pk)
            sql += "\n order by %s" % orderby
    ...
    def select_as_dict(
        self, attr='*', from_expr=None, where="", vals=None, orderby="", cursor=None, **kwargs
    ):
    ...
    def exists(self, cursor=None):
        return self.select_one(cursor=cursor, **{k: self[k] for k in self.pk}) is not None
    ...
    def insert(self, reload=True, cursor=None, **kwargs):
        ...
        self.before_insert()
        keys = [key for key in self.keys() if key[0] != '_']
        ...
        if reload == True:
            if 'sqlite' in self.db.servername().lower():
                d = self.db.select_one(
                    "select * from %s where ROWID=last_insert_rowid()" % self.relation
                )
            elif 'postgres' not in self.db.servername().lower() and None not in [
                self.get(k) for k in self.pk
            ]:  # local pk is filled
                whereargs = {}
                for k in self.pk:
                ...
                d = self.db.select_one(
                    "select * from %s where %s" % (self.relation, wherecl), vals=wherevals
                )
            for k in list((d or {}).keys()):
                self[k] = d[k]
        if cursor is None:
            self.db.commit()
        self.after_insert()
    ...
    def delete(self, where=None, vals=None, cursor=None):
        ...
        if where == None:
            ...
            vals += addvals
        self.db.execute(
            "delete from %s where %s" % (self.relation, where), vals=vals, cursor=cursor
        )
    ...
    def after_insert_or_update(self):
        pass
